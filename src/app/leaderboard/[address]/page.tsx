            ...
            {isConnected ? (
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 bg-[#111] border border-[#333] px-3 py-1.5 hover:border-[#00FF66] transition-colors group">
                  ...
                  <ChevronDown className="w-3 h-3 text-[#6B6B6B] group-hover:text-[#00FF66]" />
                </button>
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#111] border border-[#333] shadow-[0_0_20px_rgba(0,0,0,0.5)] flex flex-col font-mono text-xs z-50 animate-fade-in-up" style={{animationDuration:'0.2s'}}>
                    <button className="flex items-center gap-2 px-4 py-3 text-[#6B6B6B] hover:text-white hover:bg-[#222] text-left"><Copy className="w-3 h-3" /> Copy Address</button>
                    <button className="flex items-center justify-between px-4 py-3 text-[#6B6B6B] hover:text-white hover:bg-[#222] text-left border-t border-[#222]"><span className="flex items-center gap-2"><ExternalLink className="w-3 h-3" /> Etherscan</span><ArrowUpRight className="w-3 h-3" /></button>
                    <button className="flex items-center gap-2 px-4 py-3 text-[#6B6B6B] hover:text-white hover:bg-[#222] text-left border-t border-[#222]"><Activity className="w-3 h-3" /> Switch Network</button>
                    <button onClick={() => { setIsConnected(false); setDropdownOpen(false); }} className="flex items-center gap-2 px-4 py-3 text-[#FF3B3B] hover:bg-[#FF3B3B]/10 text-left border-t border-[#222]"><Power className="w-3 h-3" /> Disconnect</button>
                  </div>
                )}
              </div>
            ) : (

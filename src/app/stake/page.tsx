      ...
      <nav className="sticky top-0 z-50 bg-[#0A0A0A]/95 backdrop-blur-sm border-b border-[#222]">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map(l => (
              <a key={l.href} href={l.href} className={`font-mono text-sm transition-colors ${l.href === '/stake' ? 'text-[#00FF66]' : 'text-[#6B6B6B] hover:text-white'}`}>{l.label}</a>
            ))}
          </div>
        ...
        {mobileMenuOpen && (
          <div ref={menuRef} className="md:hidden bg-[#0A0A0A] border-t border-[#222] px-4 py-4 space-y-3">
            {navLinks.map(l => (
              <a key={l.href} href={l.href} className={`block font-mono text-sm ${l.href === '/stake' ? 'text-[#00FF66]' : 'text-[#6B6B6B]'}`}>{l.label}</a>
            ))}
          </div>
        )}
      </nav>
          ...
          <>
            ...
            <div className="bg-[#111] border border-[#222] p-6 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex gap-3">
                <a href="/token" className="bg-transparent border border-[#333] text-white font-mono text-xs font-bold px-5 py-2.5 hover:border-[#00FF66] transition-colors flex items-center gap-2">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0A0A0A] border-t border-[#222] z-50">
        <div className="flex justify-around py-2">
          {bottomNavItems.map(item => (
            <a key={item.href} href={item.href} className={`flex flex-col items-center gap-0.5 px-3 py-1 ${item.href === '/stake' ? 'text-[#00FF66]' : 'text-[#6B6B6B]'}`}>
              {item.icon}
